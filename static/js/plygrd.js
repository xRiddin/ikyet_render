...
const user_image = `<img src="${url_prefix}/static/images/user.png" alt="User Avatar">`;
const gpt_image = `<img src="${url_prefix}/static/images/gpt.png" alt="GPT Avatar">`;
...
const ask_gpt = async (message) => {
	try {
		...
		const response = await fetch('/playgrd', {
			method: 'POST',
			headers: {
			...
			body: JSON.stringify({
				...
				userText: message,
				web: document.getElementById("switch").checked,
		...
		if (responseText.includes("https://replicate.delivery")) {
			...
			document.getElementById(`gpt_${window.token}`).innerHTML = markdown.render(responseText);
			...
			if (responseText.includes(`instead. Maintaining this website and API costs a lot of money`)) {
